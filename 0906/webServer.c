...
void *clnt_connection(void *arg)
{
    ...
    /* 요청을 구분하여 출력 */
    /* 문자의 위치를 찾을 때 사용하는 함수 strchr */
    char *params = strchr(filename, '?');  /* 파라미터 시작점 찾기 */
    char *param_key;                       /* 파라미터가 여러 개인 부분을 할당하는 초기 변수 */

    if (params != NULL) {
        *params = '\0';                    /* 파일명과 파라미터 분리 (하지 않으면 첫 줄 GET 출력시 이어서 붙여서 출력이 됌.)*/
        params++;                          /* 파라미터 부분을 가리킴 */

        /* 첫 번째 줄: GET /index.html */
        printf("GET %s \n", filename);

        /* 파라미터가 여러 개인 경우 모두 출력 */
        /* 문자열 토근을 구분하기 위해서 사용하는 함수 : strtok */
        while ((param_key = strtok(params, "&=")) != NULL) {
            printf("%s\n", param_key);
            params = NULL;                  /* 이후 strtok() 호출 시에는 NULL로 설정 */
        }
